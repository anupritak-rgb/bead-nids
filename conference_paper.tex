...
\title{Statistical Anomaly Detection in Encrypted Network\\
Traffic Using Benford's Law and Entropic Hybrid Analysis}
...
By combining both metrics into a unified Hybrid Anomaly Score, the
proposed Entropic Benford Hybrid Engine closes these individual gaps:
...
capability across all four canonical attack categories, with the Hybrid
Engine retaining an F1-score of 0.891 under synthetic mimicry conditions
where single-dimensional Benford analysis degrades to 0.612.
\end{abstract}
...
\begin{itemize}
  \item A formally specified four-layer modular NIDS architecture processing
        ...
        flow-level metadata.
  \item A novel \textit{Entropic Benford Hybrid Engine} combining
...
\node[sbox, fill=red!12]    at ( 1.37, -4.50) {Entropy\\Hybrid Engine};
...
\subsection{Entropic Benford Hybrid Engine}
\label{sec:hybrid}
...
Hybrid Engine exploits a key mathematical constraint: an attacker who
...
$H_B \approx 3.134$~bits. The \textit{Hybrid Anomaly Score} is:
\begin{equation}
...
\begin{table}[t]
\caption{MAD Scores by NSL-KDD Traffic Category}
\label{tab:mad}
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Traffic Category} & \textbf{Mean MAD} & \textbf{Conformity Status} \\
\midrule
Normal              & 0.009 & Acceptable      \\
DoS (Neptune/Smurf) & 0.047 & Non-conforming  \\
Probe (PortScan)    & 0.026 & Non-conforming  \\
R2L (FTP Write)     & 0.019 & Marginal--NC    \\
U2R (Buffer Overflow) & 0.031 & Non-conforming \\
\bottomrule
\multicolumn{3}{l}{\footnotesize NC = Non-conforming ($>$0.015); per Nigrini~\cite{b6}.}
\end{tabular}
\end{table}

Normal traffic produces MAD values in the acceptable range
($0.006$--$0.012$), while all attack categories exceed the $0.015$
non-conformance threshold. DoS attacks exhibit the highest deviation
($\text{MAD} \approx 0.047$) due to fixed-size flooding packets.
Probe attacks show intermediate deviation ($\text{MAD} \approx 0.026$),
consistent with the partially systematic nature of port-scanning behavior.
...
RF, 41 features$^{\dagger}$~\cite{b17} &
...
\textbf{Proposed Hybrid (label-free)} &
  ...
  \shortstack[c]{$\mathbf{0.058}$\\[-2pt]{\scriptsize$\pm.004$}} \\
\bottomrule
\multicolumn{5}{@{}p{7.8cm}@{}}{\scriptsize
  $^{\dagger}$Supervised; uses labeled training data.
  Included for context only.}
\end{tabular}
\end{table}

The Hybrid Engine achieves F1 of 0.940 without any labeled training, a
6.8 percentage-point improvement over the single-dimensional Benford MAD
baseline. The gap between the proposed label-free method and supervised
RF/LSTM ($\Delta\text{F1} \approx 0.006$) is within statistical overlap,
demonstrating competitive accuracy without the labeled-data dependency.
...
distributions conform to Benford expectations. The Hybrid Engine retains
...
hardware, the Hybrid Engine sustains 148,000 packets per second throughput
...
\subsection{Short-Flow Detection Latency}

The sliding-window mechanism requires a minimum of approximately 1000 packets
to achieve statistically reliable conformity scores. Short-duration attack
flows, such as rapid SYN floods, are detectable only after the window fills,
introducing a bounded detection latency proportional to the inter-arrival
spacing. Partial-window early-warning triggers calibrated to a lower
statistical threshold can partially mitigate this limitation at the cost of
an increased false-positive rate.

\subsection{Encryption Agnosticism}

Because the proposed framework processes only flow-level metadata, specifically
packet sizes visible at the transport layer and inter-arrival timestamps
recorded by the capture engine, it operates identically on plaintext and
TLS~1.3 encrypted flows. The QUIC protocol similarly cannot obscure packet
sizes or timing at the IP layer~\cite{b10}. This property is architecturally
guaranteed and does not degrade as encryption standards evolve.

\subsection{Explainability Advantage}

Each detection event is accompanied by a decomposed anomaly report specifying
$\chi^2$, MAD, and entropy-deviation scores individually, allowing SOC
analysts to distinguish DoS attacks (high MAD, moderate entropy deviation)
from mimicry attempts (low MAD, high entropy deviation) without consulting
opaque model internals, directly addressing the interpretability concerns
surveyed in~\cite{b18}.
...
principled resistance against mimicry attacks. The Entropic Benford Hybrid
Engine constructs a two-dimensional evasion barrier that adversaries cannot
...
detection,''
in \textit{Proc. NDSS Symp.}, San Diego, CA, USA, Feb. 2018.
...
features,''
in \textit{Proc. 2nd Int. Conf. Inf. Syst. Secur. Privacy (ICISSP)},
Rome, Italy, 2016, pp.~407--414.
