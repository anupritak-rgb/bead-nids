...
NSL-KDD~\cite{b9} predates TLS~1.3 and QUIC; results therefore validate
statistical separability of Benford scores rather than full operational
performance on contemporary encrypted traffic. Packet-size reconstruction
from per-connection byte counts introduces bounded approximation error,
and the UNSW-NB15 cross-validation (Table~\ref{tab:unswnb15}) partially
addresses the dataset-currency gap. Enterprise networks with high
bulk-transfer proportions (backups, media delivery) may exhibit lower
baseline Benford conformity, elevating false-positive rates; a $\sim$24-hour
site-specific profiling phase before activating detection is recommended.

\subsection{Adversarial Threat Model}

Under full Kerckhoffs-style adversary transparency, evasion requires
simultaneously satisfying: (1) $p_i \approx b_i$ for all
$i \in \{1,\ldots,9\}$ to keep MAD~$<0.015$; and (2)
$|H_{\text{obs}} - H_B| < \epsilon$ to suppress the entropy term.
Condition~(1) fixes the digit-1 frequency to $\approx 30\%$; condition~(2)
constrains the full distributional shape. Any optimization-based sampler
satisfying both imposes non-trivial per-flow overhead, and the induced
jitter perturbs IATs which the KS test exploits as a secondary channel.
Formal evasion-cost analysis remains a future direction.

\subsection{Window-Size Sensitivity}

Table~\ref{tab:window} reports F1 and FPR across window widths on NSL-KDD.
...
F1 gains plateau beyond $w=1000$ while latency grows linearly; $w=500$
suits latency-sensitive deployments at a modest precision cost.

\subsection{Encryption Agnosticism and Explainability}

BEAD processes only transport-layer metadata and therefore operates
identically on plaintext and TLS~1.3/QUIC flows~\cite{b10}. Per-alert
decomposition into $\chi^2$, MAD, and entropy-deviation scores enables
SOC analysts to distinguish DoS (high MAD) from mimicry attempts
(low MAD, elevated entropy deviation)~\cite{b18}.
